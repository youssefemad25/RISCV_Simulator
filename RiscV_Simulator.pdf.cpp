...
void printRegistersAndMemory(const string& File) {
    // Print Registers
    ...
    for (int i = 0; i < 32; i++) {
        ...
        outFile << "R" << i << ": " << registers[i] << endl;
    }

    // Print Memory
    cout << "\nFinal State of Memory:\n";
    outFile << "\nFinal State of Memory:\n";
    for (const auto& mem : memory_access) {
        cout << "Address: " << mem.first << " -> Value: " << mem.second << endl;
        outFile << "Address: " << mem.first << " -> Value: " << mem.second << endl;
    }
    outFile.close();
}
...
int main()
{
    ...
    string inputfilepath = "C:\\Users\\youss\\OneDrive - aucegypt.edu\\Desktop\\in.txt";
    //cin >> inputfilepath;
    cout << "enter initial program count: " ;
    cin >> progcount;
    ...
    printRegistersAndMemory(outputfilepath);
